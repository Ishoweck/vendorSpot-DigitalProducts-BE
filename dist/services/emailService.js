...
class EmailService {
    ...
    constructor() {
        if (config_1.config.resendApiKey) {
            this.resend = new resend_1.Resend(config_1.config.resendApiKey);
            this.isResendEnabled = true;
            logger_1.logger.info("Resend email service initialized");
        }
        else {
            logger_1.logger.warn("Resend API key not provided. Email service will use fallback method.");
        }
    }
    async sendEmail(options) {
        try {
            if (this.isResendEnabled && this.resend) {
                const emailData = {
                    from: options.from || config_1.config.emailFrom,
                ...
                logger_1.logger.info("Email would be sent (Resend not configured)", {
                    to: options.to,
                    subject: options.subject,
                    from: options.from || config_1.config.emailFrom,
    ...
    async sendVerificationOTPEmail(to, firstName, verificationOTP) {
        const html = `
    ...
    async sendOrderConfirmationEmail(to, firstName, orderDetails) {
        const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background-color: #f8f9fa; padding: 30px; border-radius: 10px;">
          ...
          <div style="margin-top: 30px;">
            <a href="${config_1.config.frontendUrl}/dashboard/notifications" 
